...
var CloudinaryVideoField = function () {
	...
	field.ReferenceName = "CloudinaryMedia";
	field.Render = function (options) {
		...
		var $pnl = $(".cloudinary-field", options.$elem);

		var cloudinarySettings = {
			cloud_name: 'agility-cms',
			api_key: '927841219782672'
		}
		...
		if ($pnl.size() == 0) {
			...
			//pull down the html template and load it into the element
			$.get("https://localhost:3000/cloudinary-video.html", function (htmlContent) {

				options.$elem.append(htmlContent)

				$pnl = $(".cloudinary-field", options.$elem);
				...
				var viewModel = new function () {
					...
					self.frameSrc = ko.computed(function() {
						if (self.fieldBinding().public_id() == null) {
							...
							return "https://localhost:3000/cloudinary-player.html?id="+ self.fieldBinding().public_id()
						}
					})
